...
module.exports = {
  ...
  async execute({ interaction }) {
    ...
    if (!cfg.femaleRoleId) {
      await interaction.reply({ content: 'Kadın rolü ayarlı değil. `/kayit-ayar` kullan.', ephemeral: true });
      return;
    }
    ...
    await member.roles.add(cfg.femaleRoleId).catch(() => null);

    await incRegistrar(interaction.guildId, interaction.user.id, 'female');
